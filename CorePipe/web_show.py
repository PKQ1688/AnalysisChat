...
with gr.Blocks() as app:
    memory = gr.State()
    gr.HTML(
        """<center><h1 style='font-size:xx-large;'>模型能力测试</h1><br><h3>生机智能大模型</h3><br><h7>""")
    chat_b = gr.Chatbot(height=100)
    with gr.Group():
        with gr.Row():
            with gr.Column(scale=3):
                ...
                with gr.Accordion("Prompt Format", open=False):
                    custom_prompt = gr.Textbox(label="Modify Prompt Format",
                                               info="For testing purposes. 'USER_INPUT' is where 'SYSTEM_PROMPT, PROMPT' will be placed",
                                               lines=3,
                                               value="<start_of_turn>userUSER_INPUT<end_of_turn><start_of_turn>model")
                with gr.Row():
            ...
            with gr.Column(scale=1):
                with gr.Group():
                    ...
                    tokens = gr.Slider(label="Max new tokens", value=1600, minimum=0, maximum=8000, step=64,
        ...
        with gr.Accordion(label="Screenshot", open=False):
            with gr.Row():
                ...
                with gr.Column(scale=1):
                    ...
                    chatblock = gr.Dropdown(label="Chatblocks", info="Choose specific blocks of chat",
                                            choices=[c for c in range(1, 40)], multiselect=True)
    ...
    chat_sub = inp.submit(check_rand, [rand, seed], seed).then(chat_inf,
                                                               [sys_inp, inp, chat_b, memory, client_choice, seed, temp,
                                                                tokens, top_p, rep_p, chat_mem, custom_prompt],
                                                               [chat_b, memory])
